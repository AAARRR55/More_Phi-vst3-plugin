...
|---|------|-----------|--------|-------|
| 8.1.1 | Plugin scan | Preferences > Plug-ins > rescan | ☐ PASS ☐ FAIL | MorphSnap appears with correct name and version |
| 8.1.2 | Instantiation | Drag MorphSnap to audio track | ☐ PASS ☐ FAIL | Must load without crash; UI renders correctly |
...
| 8.1.7 | Multi-instance (4x) | 4 tracks, each with MorphSnap + different hosted preset | ☐ PASS ☐ FAIL | Independent state per instance, no cross-talk |
...
| # | Test | Procedure | Result | Notes |
|---|------|-----------|--------|-------|
| 8.3.1 | Device view | Open MorphSnap in Detail/Device view | ☐ PASS ☐ FAIL | All parameters visible in device chain |
...
| 8.3.3 | Freeze/flatten | Freeze track with MorphSnap, then unfreeze | ☐ PASS ☐ FAIL | State must be fully preserved after unfreeze |
| 8.3.4 | Macro mapping | Map morph position to macro knob, move macro | ☐ PASS ☐ FAIL | Macro must control morph position |
| 8.3.5 | Group device | Place MorphSnap in a device group | ☐ PASS ☐ FAIL | Full functionality within group — no routing breakage |
