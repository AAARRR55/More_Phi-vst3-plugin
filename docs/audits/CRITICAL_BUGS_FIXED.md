...
## 2026-06-24 Neural Mastering Audit — Bridge Implementation

Thirteen actionable fixes from the neural→hosted-plugin bridge audit. The bridge
forwards `ValidatedNeuralMasteringPlan` decisions to the hosted Ozone (or similar)
mastering plugin via `chainPlanner_.applyPlan()`, while the internal DSP chain
remains dormant as an optional preview path.

### CRITICAL fixes
- **CRITICAL-1:** `peakAbs()` in `SonicMasterAnalysisEngine.cpp` now skips
  non-finite samples (`std::isfinite` guard). Defensive check on the resulting
  peak prevents NaN/Inf from poisoning inference gain (`runCycle`, `requestDecisionNow`).
- **CRITICAL-6/7/17:** `applyValidatedPlan()` now builds a `MultiEffectPlan` via
  `buildBridgePlanFromNeural()` and forwards it through `chainPlanner_.applyPlan()`.
  The Ozone parameter enqueue count is stored in `lastOzoneAppliedCount_`.
- **CRITICAL-13:** Heuristic `applyPlan()` re-entrant clobber lines (limiter ceiling,
  exciter enable) now guarded by `neuralHasLimiter`/`neuralHasHarmonic` checks.

### HIGH fixes
- **HIGH-2:** True-peak accuracy disclaimer echoed in PRODUCT_POSITIONING.md and
  this file: the `TruePeakEstimator` 12-tap prototype under-reads near-Nyquist by
  ~20-25 dB (see `TruePeakEstimator.h:17-27`). Not ±0.2 dBTP.
- **HIGH-13:** Clobber guards (see CRITICAL-13 above).
- **HIGH-18/19:** `applied_to_audio_path` now reads `engine.lastApplyReachedAudioPath()`
  instead of `engine.isActive()` directly, so the field reflects reality whether
  audio reaches the internal chain or the hosted plugin bridge.
- **`reachedAudio` redefinition:** `SonicMasterAnalysisEngine::runCycle()` now
  computes `reachedAudio = internalActive || ozoneWrote`, preserving both the
  L109 (dormant+no-applicator → AppliedNoAudioPath) and L292 (active chain → Applied)
  test pinning.

### MED fixes
- **MED-11:** Mastering plan persistence via `serializeLastPlan`/`restoreLastPlan`
  (base64 blit of the POD `ValidatedNeuralMasteringPlan`). Wired in
  `PluginProcessor.cpp` save/restore as a `MASTERING_PLAN` XML child.
- **MED-12:** All MCP meter reads wrapped in `finiteOr()`: -70 LUFS sentinel for
  loudness, -300 dBTP / -∞-equivalent for peak/GR fields.

### LOW fixes
- **LOW-4:** ONNX→HTTP automatic failover. `SonicMasterAnalysisEngine` gains
  `setFallbackInferenceSource()`. When the primary source hits the consecutive
  failure limit and a fallback is wired, it swaps instead of disabling.
- **LOW-3/5:** Documentation: ORT 1.22.1 pinning note in CMakeLists.txt;
  MED-10/20 ponytail notes for instant-set safety and inter-stage clip risk.
