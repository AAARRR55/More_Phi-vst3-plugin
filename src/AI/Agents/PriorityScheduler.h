// src/AI/Agents/PriorityScheduler.h
#pragma once
...
private:
    ...
    // AUDIT C-1 (2026-06-30): std::mutex + std::condition_variable_any is the
    // CORRECT primitive here, not a defect to swap for juce::SpinLock. Rationale:
    //   1. Workers SLEEP on cv_.wait(lock, stopToken, ...) — zero CPU when idle.
    //      A SpinLock busy-waits, which would burn a core per worker for no gain.
    //   2. cv_.wait's stop_token overload (jthread cooperative cancellation) is a
    //      std::condition_variable_any API that requires a unique_lock<std::mutex>;
    //      juce::SpinLock is incompatible with this wait pattern.
    //   3. No priority-inversion risk exists: all scheduler workers run at normal
    //      thread priority (no higher-priority thread contends on mutex_), and the
    //      RealtimeCritical path already bypasses this mutex entirely via the
    //      lock-free urgents slot (C-3 FIX, lines 94-103).
    // The earlier audit's "CRITICAL — replace with juce::SpinLock" was based on a
    // generic priority-inversion heuristic that does not apply to a same-priority
    // sleeping worker pool. Left as std::mutex by design.
